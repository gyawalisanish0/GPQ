...
export class MainMenuScene extends BaseScene {
  ...
  preload() {
    ...
    // FIX: Generate the shared particle textures here, in MainMenuScene.preload(),
    // so they exist before any scene tries to use them. Previously these were only
    // created inside Game_Scene.preload() which runs much later; buildUI() in this
    // scene called this.add.particles('particle') before that texture existed,
    // causing Phaser 3.90's ParticleEmitter to throw, which silently exited the
    // try-block in create() and prevented SCENE_READY from ever being emitted —
  ...
  create() {
    try {
      ...
      // FIX: Emit SCENE_READY unconditionally via finally so the React loading
      // overlay is always dismissed — even if buildUI() threw an error above.
      // Previously this emit lived at the end of the try-block, meaning any
      // exception (e.g. missing texture) silently swallowed the event and left
      // App.tsx waiting on a promise that would never resolve.
  ...
  private buildUI() {
    ...
    // Particle background — 'star_particle' now guaranteed to exist (created in preload())
